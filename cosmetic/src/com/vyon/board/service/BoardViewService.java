...
public class BoardViewService implements Service {
	...
	public ForwardInfo process(HttpServletRequest request,
		...
		try {
		    ...
		    //조회수 1증가 메소드
			dao.targetincrease(con, board_id, no);
			//해당번호 글가져오기
			Board board = dao.view(con, board_id, no);
			//con.commit();
			List<BoardReply> br = dao.replyView(con,board_id,no);
			
			request.setAttribute("data", board);
			request.setAttribute("reply", br);
		...
		forwardInfo.setForwardStr("/board/view.jsp");
		return forwardInfo;
	}
