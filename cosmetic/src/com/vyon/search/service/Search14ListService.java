...
public class Search14ListService implements Service {
...
//		Search9ListSevice에서 세션에 저장한 row 값을 불러온다
...
//			헤드부분의 검색창에서 검색을한뒤 나오는 페이지 check2와 check3이 같으면 무조건 헤드 검색창
//			에서 검색을 한다는 뜻이다
			request.setAttribute("check2", "check");
			request.setAttribute("check3", "check");
//			헤드부분에 검새창에 검색을 한뒤 여러가지 순으로 정렬을 한뒤 row개 씩 화면에
//			보여지게 한뒤 의 페이징 숫자를 클릭하게 끔하는 페이지 숫자를 클릭하면
//			Search14ListService로 이동
			request.setAttribute("check9", "check9");
			request.setAttribute("check5", "check9");
//			헤드부분의 검색창에서 검색을 한뒤 여러가지순으로 정렬을 한뒤 row개로 검색을 하는 페이지
//			Search9ListService로 넘어감
