...
public class Search4ListService implements Service {
...
//		}
		...
		System.out.println("search4ListService");
...
//			System.out.println("현재 페이지:"+page);
		...
		try {
			...
			totalRow=dao.selectCount(con,name2,select);
			//전체 페이지 구하기
			if(totalRow>0)
...
//			System.out.println("endRow:"+endRow);
			...
			// 게시판에 글을 쓴다.
			System.out.println("여기까지");
			List<Search> list=dao.search(con,startRow,endRow,name2,select,sort,method);
//			List<Goods> list
...
//			check2와 check3이 다르면 검색결과 내의 검색창에서 검색을 한것이다
			request.setAttribute("check2", "check2");
			request.setAttribute("check3", "check3");
//			request.setAttribute("check2", "check2");
			request.setAttribute("check6", "check6");
			request.setAttribute("check5","check6");
//			검색결과내의 검색창에서 검색을 한뒤 여러가지순으로
//			정렬을 한뒤 row개의 개수로 정렬할 페이지다
