package com.vyon.search.dao;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

import com.vyon.jdbc.JdbcUtil;
import com.vyon.search.model.Search;

public abstract class SearchDao {

	protected Search makeSearchFromListResultSet(ResultSet rs)
			throws SQLException {
		Search search = new Search();
		search.setCname(rs.getString("cname"));
		search.setGcode(rs.getString("gcode"));
		search.setName(rs.getString("name"));
		search.setGimage1(rs.getString("o_file"));
		search.setGimage2(rs.getString("s_file"));
		search.setPrice(rs.getInt("price"));
		search.setDisRate(rs.getInt("disrate"));
		search.setRnum(rs.getInt("rnum"));
		search.setSalePrice(rs.getInt("sale"));
		return search;
	}

	public int selectCount(Connection con, Search search, String name2)
			throws SQLException {
		// 사용할 객체 선언
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		// 처리
		System.out.println(search.getName());
		try {
			if (search.getName() != null) {
				String sql = " select g.cname,g.gcode,g.name,g.product, "
						+ " gi.o_file,gi.s_file,p.price,p.disrate, "
						+ " round(p.price*(100-p.disrate)/100,-1) sale "
						+ " from goods g,goods_img gi,price p where "
						+ " g.gcode=gi.gcode and gi.gcode=p.gcode "
						+ " and gi.img_size ='s' ";
				sql = " select rownum rnum,cname,gcode,name, "
						+ " o_file,s_file,price,disrate,product,sale "
						+ " from (" + sql + ")where 1=2 or name like ? or "
						+ " gcode like ? or product like ? ";

				sql = "select count(*) from (" + sql + ")";
				// 상태
				pstmt = con.prepareStatement(sql);
				// 실행 select: executeQuery()
				pstmt.setString(1, "%" + search.getName() + "%");
				pstmt.setString(2, "%" + search.getName() + "%");
				pstmt.setString(3, "%" + search.getName() + "%");
				rs = pstmt.executeQuery();
			} else {
				if (name2 != null) {
					String sql = " select g.cname,g.gcode,g.name,g.product, "
							+ " gi.o_file,gi.s_file,p.price,p.disrate, "
							+ " round(p.price*(100-p.disrate)/100,-1) sale "
							+ " from goods g,goods_img gi,price p where "
							+ " g.gcode=gi.gcode and gi.gcode=p.gcode "
							+ " and gi.img_size ='s' ";
					sql = " select rownum rnum,cname,gcode,name, "
							+ " o_file,s_file,price,disrate,product,sale "
							+ " from (" + sql + ")where 1=2 or name like ? or "
							+ " gcode like ? or product like ? ";

					sql = "select count(*) from (" + sql + ")";
					pstmt = con.prepareStatement(sql);
					// 실행 select: executeQuery()
					pstmt.setString(1, "%" + name2 + "%");
					pstmt.setString(2, "%" + name2 + "%");
					pstmt.setString(3, "%" + name2 + "%");
					rs = pstmt.executeQuery();
				}else{
					String sql=" select * from goods g,goods_img gi,price p  "
							+ " where g.gcode=gi.gcode and gi.gcode=p.gcode and gi.img_size ='s'" ;
					sql = "select count(*) from (" + sql + ")";
					pstmt = con.prepareStatement(sql);
					rs = pstmt.executeQuery();
				}

			}
			
			if (rs != null)
				if (rs.next())
					return rs.getInt(1);
		} finally {
			// 처리가 된 후 객체 닫기
			JdbcUtil.close(pstmt);
			JdbcUtil.close(rs);
		}
		return 0;
	}

	public abstract List<Search> search(Connection con, Search search,
			int startRow, int endRow, String name2) throws SQLException;

	public abstract List<Search> search(Connection con, Search search,
			int startRow, int endRow, String name2, String select)
			throws SQLException;

	public abstract List<Search> search(Connection con, int startRow,
			int endRow, String name2 , String select) throws SQLException;

	// TODO Auto-generated method stub

	public abstract List<Search> search(Connection con, int startRow,
			int endRow, String name2, String select, String sort, String method)
			throws SQLException;

	public int selectCount(Connection con, String name2, String select) throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<Search> list = null;
		// 처리
		try{
			System.out.println("name2는"+name2);
			System.out.println("select는"+select);
			//sql 작성
			if(select != null){
				String sql=" select g.cname,g.gcode,g.product,g.name,gi.o_file,gi.s_file,p.price,p.disrate, "
						+ " round(p.price*(100-p.disrate)/100,-1) sale "
						+ "from goods g,goods_img gi,price p where g.gcode=gi.gcode "
						+ "and gi.gcode=p.gcode  and gi.img_size ='s' " ;
				//상태
				sql=" select rownum rnum,cname,gcode,name,o_file,s_file,price,disrate,product "
						+ " from ("+sql+") where 1=2 or " + select 
					+ " like ? ";
			sql = " select count(*) from (" + sql + ") ";
			pstmt = con.prepareStatement(sql);
			// ?에 대한 데이터 셋팅
//			pstmt.setString(1, select);
			pstmt.setString(1, "%" + name2 + "%");
			
			//실행 select: executeQuery()
			rs=pstmt.executeQuery();
			}else{
				System.out.println("여기까지옴");
				String sql=" select g.cname,g.gcode,g.name,g.product, "
						+" gi.o_file,gi.s_file,p.price,p.disrate, "
						+" round(p.price*(100-p.disrate)/100,-1) sale "
						+" from goods g,goods_img gi,price p where "
						+" g.gcode=gi.gcode and gi.gcode=p.gcode "
						+" and gi.img_size ='s' ";
						sql =" select rownum rnum,cname,gcode,name, "
						+ " o_file,s_file,price,disrate,product,sale "
						+" from ("+sql+")where 1=2 or name like ? or "
						+" gcode like ? or product like ? ";
						sql = " select count(*) from (" + sql + ") ";
					pstmt = con.prepareStatement(sql);
					// ?에 대한 데이터 셋팅
					pstmt.setString(1, "%" + name2 + "%");
					pstmt.setString(2, "%" + name2 + "%");
					pstmt.setString(3, "%" + name2 + "%");
					
					//실행 select: executeQuery()
					
					rs=pstmt.executeQuery();
			}
			if (rs != null)
				if (rs.next())
					return rs.getInt(1);
		} finally {
			// 처리가 된 후 객체 닫기
			JdbcUtil.close(pstmt);
			JdbcUtil.close(rs);
		}
		return 0;
	}

	public int selectCount(Connection con, String name2)throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<Search> list = null;
		// 처리
		try{
			System.out.println("name2는"+name2);
			
				String sql=" select g.cname,g.gcode,g.product,g.name,gi.o_file,gi.s_file,p.price,p.disrate, "
						+ " round(p.price*(100-p.disrate)/100,-1) sale "
						+ "from goods g,goods_img gi,price p where g.gcode=gi.gcode "
						+ "and gi.gcode=p.gcode  and gi.img_size ='s' " ;
				//상태
				sql=" select rownum rnum,cname,gcode,name,o_file,s_file,price,disrate,product "
						+ " from ("+sql+") where 1=2 or name like ? "
					+ " or product like ? or gcode like ? ";
			sql = " select count(*) from (" + sql + ") ";
			pstmt = con.prepareStatement(sql);
			// ?에 대한 데이터 셋팅
//			pstmt.setString(1, select);
			pstmt.setString(1, "%" + name2 + "%");
			pstmt.setString(2, "%" + name2 + "%");
			pstmt.setString(3, "%" + name2 + "%");
			
			
			//실행 select: executeQuery()
			rs=pstmt.executeQuery();
		
			if (rs != null)
				if (rs.next())
					return rs.getInt(1);
		} finally {
			// 처리가 된 후 객체 닫기
			JdbcUtil.close(pstmt);
			JdbcUtil.close(rs);
		}
		return 0;
	}

	public abstract List<Search> search(Connection con, int startRow, int endRow,
			String name2) throws SQLException;

	public abstract List<Search> search(Connection con, int startRow, int endRow,
			String name2, String sort, String method) throws SQLException;

	public int selectCount(Connection con, Search search, String name2,
			String select)throws SQLException {
		// 사용할 객체 선언
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		// 처리
		System.out.println(search.getName());
		try {
			if (search.getName() != null) {
				if(select != null){
				String sql = " select g.cname,g.gcode,g.name,g.product, "
						+ " gi.o_file,gi.s_file,p.price,p.disrate, "
						+ " round(p.price*(100-p.disrate)/100,-1) sale "
						+ " from goods g,goods_img gi,price p where "
						+ " g.gcode=gi.gcode and gi.gcode=p.gcode "
						+ " and gi.img_size ='s' ";
				sql = " select rownum rnum,cname,gcode,name, "
						+ " o_file,s_file,price,disrate,product,sale "
						+ " from (" + sql + ")where 1=2 or "+ select +" like ? ";

				sql = "select count(*) from (" + sql + ")";
				// 상태
				pstmt = con.prepareStatement(sql);
				// 실행 select: executeQuery()
				pstmt.setString(1, "%" + search.getName() + "%");
				rs = pstmt.executeQuery();
				}else{
					String sql = " select g.cname,g.gcode,g.name,g.product, "
							+ " gi.o_file,gi.s_file,p.price,p.disrate, "
							+ " round(p.price*(100-p.disrate)/100,-1) sale "
							+ " from goods g,goods_img gi,price p where "
							+ " g.gcode=gi.gcode and gi.gcode=p.gcode "
							+ " and gi.img_size ='s' ";
					sql = " select rownum rnum,cname,gcode,name, "
							+ " o_file,s_file,price,disrate,product,sale "
							+ " from (" + sql + ")where 1=2 or name like ? or "
							+ " gcode like ? or product like ? ";

					sql = "select count(*) from (" + sql + ")";
					// 상태
					pstmt = con.prepareStatement(sql);
					// 실행 select: executeQuery()
					pstmt.setString(1, "%" + search.getName() + "%");
					pstmt.setString(2, "%" + search.getName() + "%");
					pstmt.setString(3, "%" + search.getName() + "%");
					rs = pstmt.executeQuery();
				}
				} else {
				if (name2 != null) {
					if(select != null){
					String sql = " select g.cname,g.gcode,g.name,g.product, "
							+ " gi.o_file,gi.s_file,p.price,p.disrate, "
							+ " round(p.price*(100-p.disrate)/100,-1) sale "
							+ " from goods g,goods_img gi,price p where "
							+ " g.gcode=gi.gcode and gi.gcode=p.gcode "
							+ " and gi.img_size ='s' ";
					sql = " select rownum rnum,cname,gcode,name, "
							+ " o_file,s_file,price,disrate,product,sale "
							+ " from (" + sql + ")where 1=2 or "+select+" like ? ";

					sql = "select count(*) from (" + sql + ")";
					pstmt = con.prepareStatement(sql);
					// 실행 select: executeQuery()
					pstmt.setString(1, "%" + name2 + "%");
					rs = pstmt.executeQuery();
					}else{
						String sql = " select g.cname,g.gcode,g.name,g.product, "
								+ " gi.o_file,gi.s_file,p.price,p.disrate, "
								+ " round(p.price*(100-p.disrate)/100,-1) sale "
								+ " from goods g,goods_img gi,price p where "
								+ " g.gcode=gi.gcode and gi.gcode=p.gcode "
								+ " and gi.img_size ='s' ";
						sql = " select rownum rnum,cname,gcode,name, "
								+ " o_file,s_file,price,disrate,product,sale "
								+ " from (" + sql + ")where 1=2 or name like ? or "
								+ " gcode like ? or product like ? ";

						sql = "select count(*) from (" + sql + ")";
						pstmt = con.prepareStatement(sql);
						// 실행 select: executeQuery()
						pstmt.setString(1, "%" + name2 + "%");
						pstmt.setString(2, "%" + name2 + "%");
						pstmt.setString(3, "%" + name2 + "%");
						rs = pstmt.executeQuery();
					}
				}else{
					String sql=" select * from goods g,goods_img gi,price p  "
							+ " where g.gcode=gi.gcode and gi.gcode=p.gcode and gi.img_size ='s'" ;
					sql = "select count(*) from (" + sql + ")";
					pstmt = con.prepareStatement(sql);
					rs = pstmt.executeQuery();
				}

			}
			
			if (rs != null)
				if (rs.next())
					return rs.getInt(1);
		} finally {
			// 처리가 된 후 객체 닫기
			JdbcUtil.close(pstmt);
			JdbcUtil.close(rs);
		}
		return 0;
	}

}
