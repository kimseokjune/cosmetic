...
public class OrderDaoProvider {
	...
	private static OrderDaoProvider instance = new OrderDaoProvider();

	// 3. 생성된 인스턴를 받아가기 위한 getter를 만든다.
	public static OrderDaoProvider getInstance() {
		// System.out.println
	...
	private OrderDaoProvider() {
	}

	// 사용하고자 하는 모든 dbms의 dao 프로그램을 생성해서 저장해 놓는다.
	private OrderDao mysqlDao = new OrderDao();
	private OrderDao oracleDao = new OrderDao();
	private OrderDao mssqlDao = new OrderDao();
