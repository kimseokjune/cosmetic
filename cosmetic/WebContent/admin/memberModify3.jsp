...
<body>
	...
	<div class="layer_wrapper layer_box_effect">
		...
		<div class="layer_content">
			<form action="../admin/admin2SearchProcess.do" method="post"
				name="form">
				<ul id="search">
				...
				<table >
					...
					<c:forEach var="bean" items="${data.list}">
						<tr>
							...
							<td class="list"><input type="button" value="수정"
								onclick="location.href='../admin/adminProcess.do?id=${bean.id }&email=${bean.email }'" /></td>
			...
			<div id="conpage_align">
				...
				&nbsp; <a href="admin7Process.do?page=1">[처음]</a>&nbsp;
				<c:if test="${data.startPage > 1 }">
							<a href="admin7Process.do?page=${data.page-data.pagePerGroup }">&lt;&lt;[이전페이지그룹]</a>&nbsp;
				</c:if>
						<!-- 			이전페이지 이동 -->
						<c:if test="${data.page != 1 }">
							<a href="admin7Process.do?page=${data.page-1 }">&lt;</a>&nbsp;
	 			</c:if>
						...
						<c:forEach var="page" begin="${data.startPage}"
							...
							<c:choose>
								...
								<c:otherwise>
									<a href="admin7Process.do?page=${page }">[${page }]</a>&nbsp;
					</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:if test="${data.endPage<data.totalPage}">
							<a href="admin7Process.do?page=${data.page+data.pagePerGroup }">&gt;&gt;[다음페이지그룹]</a>&nbsp;
				</c:if>
						<a href="admin7Process.do?page=${data.totalPage }">[끝]</a>&nbsp;
</c:when>
					<c:otherwise>
				&nbsp; <a href="admin8Process.do?page=1">[처음]</a>&nbsp;
				<c:if test="${data.startPage > 1 }">
							<a href="admin8Process.do?page=${data.page-data.pagePerGroup }">&lt;&lt;[이전페이지그룹]</a>&nbsp;
				</c:if>
						<!-- 			이전페이지 이동 -->
						<c:if test="${data.page != 1 }">
							<a href="admin8Process.do?page=${data.page-1 }">&lt;</a>&nbsp;
	 			</c:if>
						<!--			페이지 그룹의 시작 페이지부터 끝 페이지까지 반복 -->
						<c:forEach var="page" begin="${data.startPage}"
							end="${data.endPage }">
							<c:choose>
								...
								<c:otherwise>
									<a href="admin8Process.do?page=${page }">[${page }]</a>&nbsp;
					</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:if test="${data.endPage<data.totalPage}">
							<a href="admin8Process.do?page=${data.page+data.pagePerGroup }">&gt;&gt;[다음페이지그룹]</a>&nbsp;
				</c:if>
						<a href="admin8Process.do?page=${data.totalPage }">[끝]</a>&nbsp;
</c:otherwise>
				</c:choose>
